...
- Strengthened recovery with a directly loaded data guard, a validated IndexedDB snapshot ring, and a recovery view that can inspect and deliberately restore those snapshots. Every in-app restore/import takes and byte-verifies a pre-operation backup, byte-verifies the staged replacement, rolls back on failure, preserves the current sync identity, invalidates delayed same/cross-tab cloud work, and forces cloud sync off until the result is verified. Recovery downloads strip temporary device auth tokens, malformed records cannot be exported raw, and accidental older-total imports are blocked.
- Session History minute edits now add or claw back the exact deterministic loot entitlement at every 5/15/25/50/90/120-minute boundary. Re-crossing restores the same reward instead of rerolling it; time-only entries remain loot-free. Existing XP, coins, egg incubation, Focus Targets, Orbs, materials, and crop growth continue to follow the same canonical session correction. Legacy rewarded sessions that predate stored coin provenance now infer and audit their prior 1x/stored-multiplier baseline instead of duplicating coins on edit.
- Reorganized progression into one compact Browse menu. World, Challenges, Vault, Expedition, Focus Targets, Inventory, Bestiary, Incubator, Trophy Room, Achievements, Analytics, Recap, and History now switch within the same workspace. Removed the duplicate Focus Targets card from Log and added a target-reward history view.
- Completed the visible World, Challenges, and Vault surfaces over their existing state. World unlocks now charge each zone's advertised shard cost atomically; Challenges derive and persist progress from authoritative history and award their declared XP/shards once; Vault operations reject equipped/conflicting items. Cloud Claim/merge now preserves remote World, Challenges, Crystal Shards, mount-family progress, achievements, and Vault locations without duplicating an item between inventory and protected storage.
- Hardened Expedition farming without changing reward rates: removed destructive 1,000-event truncation, fixed stale references around planting/spending/harvesting/unlocking, added ledger and plot validation, collision-resistant operation IDs, exact yield/time displays, accessibility labels, affordability, Harvest all, resource guidance, and recent harvest history.
- Enlarged the live-session custom time control and replaced separate Lock-in/Priority cancellation controls with one shared **Cancel run · 0 credit** action. Priority's completion checkpoint remains intact.
- Updated the PWA cache to include every new runtime module. A release cache is published only after every asset is fetched, validated, fully consumed, and stored; a failed install keeps the prior complete build, and activation deletes only superseded Focus Hero caches. `sw.js` BUILD_ID -> `fh-2026-07-22-v10-7-0-resilient-adventure`; manifest version -> `10.7.0`.
- Safety boundary: no signed-in production profile or its browser storage, production cloud row, private backup, credential, or sync code was read or changed during development. Tests used isolated synthetic localhost storage and mocked cloud endpoints. Concurrent edits made offline on multiple devices remain outside the safe merge boundary and should not be treated as supported.
